...
app.post("/", async (c) => {
  ...
  return stream(c, async (stream) => {
    try {
      ...
      if (userPrompt.trim().includes("feature-request")) {
        ...
        stream.write(createTextEvent(`No problem ${user.data.login}!\n\n`));
      ...
      stream.write(
        createTextEvent(
          `Hi ${user.data.login}! The options you have are asking me about a feature request or a product idea. Type 'feature-request' to get a wacky product idea!`
        )
      );
